...
export const listCar1 = asyncErrorHandler(async (req, res, next) => {
  ...
  const { make, model, variant, year, sellerDetail } = req.body;
...
export const listCar2 = asyncErrorHandler(async (req, res, next) => {
  ...
  const { make, model, sellerDetail } = req.query;
  ...
  const car = await Car.findOneAndUpdate(
    { sellerDetail: sellerDetail, model: model, make: make },
    {
      price,
      fuel,
      transmission,
...
export const listCar3 = asyncErrorHandler(async (req, res, next) => {
  const { make, model, location, sellerDetail } = req.query;
  // const user = req.user;
  // const userId = user._id
  ...
  const car = await Car.findOneAndUpdate(
    { sellerDetail:sellerDetail, model: model, make: make, location: location },
    {
      engine,
      mileage,
      seatingCapacity,
...
export const listCar4 = asyncErrorHandler(async (req, res, next) => {
  const { model, location, registrationNumber, sellerDetail } = req.query;

  const { sellerName, sellerEmailId, sellerContactNo, sellerWhatsappNo, sellerLocation } = req.body;
  if (!sellerName || !sellerEmailId || !sellerContactNo || !sellerWhatsappNo || !sellerLocation) {
    return next(new ErrorHandler("Fill the credentials", 400));
  }
  // const user = req.user
  // const userId2 = user._id
  ...
  const car = await Car.findOneAndUpdate(
    ...
    {
      $set: {
        'specifications.sellerName': sellerName,
        'specifications.sellerEmailId': sellerEmailId,
        'specifications.sellerContactNo': sellerContactNo,
        'specifications.sellerWhatsappNo': sellerWhatsappNo,
...
export const bookATestDrive = asyncErrorHandler(async (req, res, next) => {
  ...
  car.testDrive.push({ approved: false, day, time, name, phone });
  await car.save();
