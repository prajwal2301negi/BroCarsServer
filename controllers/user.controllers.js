...
async function sendVerificationCode(email, verificationCode, res) {
    ...
    try {
        ...
        console.error('Error sending verification code via email:', error.message);
...
export const registerUser = asyncErrorHandler(async (req, res, next) => {
    ...
    //     email 
    // });
    // if (registrationAttempts.length > 300) {
    //     return next(new ErrorHandler("Too many registration attempts", 400));
    // }
    ...
    const verificationCodeExpire = Date.now() + 30 * 60 * 1000; // 30 minutes
...
//   console.log("findUser.password", findUser.password)
  const isPasswordCorrect = bcrypt.compareSync(oldPassword, findUser.password)
  if (!isPasswordCorrect) {
